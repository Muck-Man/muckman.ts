...
export default (<Command.CommandOptions> {
  ...
  run: async (context, args) => {
    ...
    const embeds: Array<Utils.Embed> = [];

    for (let activityId = 0; activityId < (activities.length || 1); activityId++) {
        ...
        if (activityId in activities) {
          ...
          if (activity) {
            ...
            if (1 < activities.length) {
              name = `Activity (${activityId + 1} of ${activities.length})`;
            }
    ...
    const page = Math.max(1, Math.min(args.id, embeds.length));
    if (1 < embeds.length && context.message.canReact) {
      const paginator = new Paginator(context, {page, pages: embeds});
      await paginator.start();
    } else {
      const embed = <any> embeds[page - 1];
